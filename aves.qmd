...
for (i in seq_len(nrow(av))) {
  ...
  cat(sprintf(
'<div class="ave-card" data-res="%s" data-buscar="%s"><div class="ave-foto">%s</div><div class="ave-info"><h4>%s</h4><div class="sci">%s</div><span class="ave-badge">%s</span>%s</div></div>\n',
    esc(a$res_norm), esc(buscar), foto,
    esc(a$especie),
...
(function(){
  ...
  function aplica(){
    var qs=(q.value||"").toLowerCase(), vr=fr.value, n=0;
  ...
  [q,fr].forEach(function(e){e.addEventListener("input",aplica);e.addEventListener("change",aplica);});
  aplica();
})();
</script>')
```
